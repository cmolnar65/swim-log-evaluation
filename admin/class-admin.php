...
final class Admin {
	...
	public function workouts() {
		...
		if($detail){
			...
			<tr><th><?php esc_html_e('Pool','swim-log-evaluation'); ?></th><td><?php echo esc_html(null!==$w->original_pool_length?$this->display_measurement($w->original_pool_length,$w->pool_length_unit):'—'); ?></td></tr>
	...
	public function upload() {
		...
		<?php foreach($pending as $p):
			...
			if(!$nw){if(!is_file($row->stored_path)){$review_error=__('The preserved source file is missing. This pending review cannot be completed until the source is restored.','swim-log-evaluation');}else{$parser=$row->source_type==='fit'?new FIT_Importer():new CSV_Importer();$reparsed=$parser->parse($row->stored_path);if(is_wp_error($reparsed))/* translators: 1: source file type (FIT or CSV), 2: parser error message. */
					$review_error=sprintf(__('The preserved %1$s source could not be reparsed: %2$s','swim-log-evaluation'),strtoupper($row->source_type),$reparsed->get_error_message());else{$valid=Importer::validate($reparsed);if(is_wp_error($valid))$review_error=$valid->get_error_message();else$nw=$reparsed['workout'];}}}
			...
			<p><strong><?php /* translators: %s: source file type (FIT or CSV). */ echo esc_html(sprintf(__('Uploaded source: %s','swim-log-evaluation'),strtoupper($row->source_type))); ?></strong> <?php /* translators: %s: localized date and time when the source was preserved. */ echo esc_html(sprintf(__('Preserved for review on %s.','swim-log-evaluation'),wp_date('F j, Y g:i a',(new \DateTimeImmutable($row->imported_at,wp_timezone()))->getTimestamp(),wp_timezone()))); ?></p>
			<?php if($review_error):?><div class="notice notice-error inline"><p><?php echo esc_html($review_error); ?></p><?php if($row->error_message):?><p><?php /* translators: %s: pending import resolution error message. */ echo esc_html(sprintf(__('Last resolution error: %s','swim-log-evaluation'),$row->error_message)); ?></p><?php endif;?></div><?php continue;endif;?>
			...
			<tr><th><?php esc_html_e('Start','swim-log-evaluation'); ?></th><td><?php echo esc_html(mysql2date('F j, Y g:i:s a',$row->candidate_start,false)); ?></td><td><?php echo esc_html(mysql2date('F j, Y g:i:s a',$nw['workout_start'],false)); ?></td><td><?php echo esc_html(number_format_i18n($start_delta).' s'); ?></td></tr>
			...
			<?php if($row->error_message):?><div class="notice notice-error inline"><p><?php /* translators: %s: pending import resolution attempt error message. */ echo esc_html(sprintf(__('Last resolution attempt: %s','swim-log-evaluation'),$row->error_message)); ?></p></div><?php endif;?>
