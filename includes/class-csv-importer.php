...
final class CSV_Importer {
	...
	public function parse($path){
		...
		if(!$wp_filesystem&&!WP_Filesystem())return new \WP_Error('swimlog_csv_read',__('WordPress could not initialize filesystem access for the CSV import.','chriss-swim-training-progress-evaluation'));
		$contents=$wp_filesystem->get_contents($path);if(false===$contents)return new \WP_Error('swimlog_csv_read',__('CSV file could not be read.','chriss-swim-training-progress-evaluation'));
		$records=str_getcsv($contents,"\n");$rows=array();foreach($records as $record){$record=rtrim($record,"\r");if(''===$record)continue;$rows[]=str_getcsv($record);if(count($rows)>20000)return new \WP_Error('swimlog_csv_large',__('CSV contains too many rows.','chriss-swim-training-progress-evaluation'));}
		...
		if(!$summary&&!$detail_header)return new \WP_Error('swimlog_csv_format',__('CSV format is not recognized as a supported swim export.','chriss-swim-training-progress-evaluation'));
