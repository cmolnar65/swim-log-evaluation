...
final class CSV_Importer {
	...
	public function parse($path){
		...
		for($i=0;$i<count($rows);$i++){
			...
			if(in_array('swimdate',$norm,true)&&in_array('swimtime',$norm,true)&&(in_array('lengthm',$norm,true)||in_array('distm',$norm,true))){$detail_header=$this->unique_keys($norm);$detail_start=$i+1;break;}
		}
