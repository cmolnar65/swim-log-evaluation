...
final class Shortcodes {
	...
	private static function stroke_name($s){return array('FR'=>__('Freestyle','swim-log-and-evaluation'),'BR'=>__('Breaststroke','swim-log-and-evaluation'),'BACK'=>__('Backstroke','swim-log-and-evaluation'),'FLY'=>__('Butterfly','swim-log-and-evaluation'),'MIXED'=>__('Mixed','swim-log-and-evaluation'))[$s]??__('Unknown','swim-log-and-evaluation');}
	private static function unavailable(){return '<div class="swimlog-public"><p>'.esc_html__('Swim results are unavailable.','swim-log-and-evaluation').'</p></div>';}
	...
	private static function distance_label($d,$c){if($c==='yd'&&$d===1650)return __('1650 yd / 1 Mile','swim-log-and-evaluation');if($c==='yd'&&$d===3300)return __('3300 yd / 2 Miles','swim-log-and-evaluation');if($c==='m'&&$d===5000)return __('5000 m / 5K','swim-log-and-evaluation');return $d.' '.$c;}
	...
	public static function latest($atts){
		...
		$count=$a['count']?max(1,min(20,$a['count'])):1;$r=Workout::recent_for_user($uid,$count);$o=self::styles().self::open($count===1?__('Latest Swim','swim-log-and-evaluation'):__('Latest Swims','swim-log-and-evaluation'),$a['title']);
		if(!$r)return$o.'<p>'.esc_html__('No swim workouts are available yet.','swim-log-and-evaluation').'</p>'.self::close();
		if($count>1)$o.='<div class="swimlog-table-wrap"><table><thead><tr><th scope="col">'.esc_html__('Date','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Location','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Distance','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Pool','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Elapsed','swim-log-and-evaluation').'</th></tr></thead><tbody>';
		...
		$o.='<dl><dt>'.esc_html__('Date','swim-log-and-evaluation').'</dt><dd>'.esc_html(self::local_date('F j, Y',$w->workout_start)).'</dd>';
		if($w->location_name)$o.='<dt>'.esc_html__('Location','swim-log-and-evaluation').'</dt><dd>'.esc_html($w->location_name).'</dd>';
		$o.='<dt>'.esc_html__('Distance','swim-log-and-evaluation').'</dt><dd>'.esc_html($dist).'</dd><dt>'.esc_html__('Pool','swim-log-and-evaluation').'</dt><dd>'.esc_html($pool).'</dd><dt>'.esc_html__('Elapsed','swim-log-and-evaluation').'</dt><dd>'.esc_html(Workout::format_duration($w->elapsed_time_ms)).'</dd>';
		if($w->primary_stroke)$o.='<dt>'.esc_html__('Primary stroke','swim-log-and-evaluation').'</dt><dd>'.esc_html(self::stroke_name($w->primary_stroke)).'</dd>';$o.='</dl>';}
	...
	public static function bests($atts){
		...
		$pbs=Records::personal_bests($uid,$course);$o=self::styles().self::open(__('Personal Bests','swim-log-and-evaluation'),$a['title']);$distances=$a['distance']?array($a['distance']):Evaluator::DISTANCES;
		...
		$o.='<div class="swimlog-table-wrap"><table><thead><tr><th scope="col">'.esc_html__('Distance','swim-log-and-evaluation').'</th>';foreach($strokes as $s)$o.='<th scope="col">'.esc_html($s==='OVERALL'?__('Overall','swim-log-and-evaluation'):self::stroke_name($s)).'</th>';$o.='</tr></thead><tbody>';
		$any=false;foreach($distances as $d){$o.='<tr><th scope="row">'.esc_html(self::distance_label($d,$course)).'</th>';foreach($strokes as $s){$p=$pbs[$d][$s]??null;$any=$any||!!$p;$o.='<td>'.($p?esc_html(Workout::format_duration($p->duration_ms)).'<br><span class="swimlog-muted">'.esc_html(self::local_date('M j, Y',$p->achieved_at)).'</span>':'&mdash;').'</td>';}$o.='</tr>';}$o.='</tbody></table></div>';return$o.(!$any?'<p>'.esc_html__('No qualifying personal bests are available for this selection.','swim-log-and-evaluation').'</p>':'').self::close();
	}
	public static function progress($atts){
		...
		$title=sprintf(__('%s — Personal Best Progression','swim-log-and-evaluation'),$event);$o=self::styles().self::open($title,$a['title']);
		if(!$rows)return$o.'<p>'.esc_html__('No personal best history is available for this event.','swim-log-and-evaluation').'</p>'.self::close();
		...
		$o.='<div class="swimlog-progress-summary"><strong>'.esc_html__('Current PB:','swim-log-and-evaluation').'</strong> '.esc_html($current['time']).' <span aria-hidden="true">·</span> <strong>'.esc_html__('First recorded PB:','swim-log-and-evaluation').'</strong> '.esc_html($first['time']).' <span aria-hidden="true">·</span> <strong>'.esc_html__('Improvement:','swim-log-and-evaluation').'</strong> '.esc_html(number_format_i18n($improvement/1000,2).' '.__('seconds','swim-log-and-evaluation')).'</div>';
		if(count($points)>1){wp_enqueue_style('swimlog-progress',SWIMLOG_EVALUATION_URL.'assets/css/progress.css',array(),SWIMLOG_EVALUATION_VERSION);wp_enqueue_script('swimlog-progress',SWIMLOG_EVALUATION_URL.'assets/js/progress.js',array(),SWIMLOG_EVALUATION_VERSION,true);$json=wp_json_encode($points);$o.='<div class="swimlog-progress-chart" data-swimlog-progress="'.esc_attr($json).'" data-swimlog-label="'.esc_attr($event).'" role="img" aria-label="'.esc_attr(sprintf(__('%s personal best progression chart. Faster times appear higher.','swim-log-and-evaluation'),$event)).'"><canvas aria-hidden="true"></canvas><div class="swimlog-progress-tooltip" hidden></div></div>';}
		$o.='<div class="swimlog-table-wrap swimlog-progress-data"><table><thead><tr><th scope="col">'.esc_html__('Date','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Personal Best','swim-log-and-evaluation').'</th></tr></thead><tbody>';foreach($points as$p)$o.='<tr><td>'.esc_html($p['date']).'</td><td>'.esc_html($p['time']).'</td></tr>';$o.='</tbody></table></div>';return$o.self::close();
	}

	public static function events($atts){
		self::load();$a=self::attrs($atts);$uid=self::owner($a);if(!self::allowed($uid))return self::unavailable();$count=$a['count']?:max(1,min(50,(int)get_option('swimlog_default_event_count',5)));$count=max(1,min(50,$count));$events=Event::upcoming_for_user($uid,$count);$o=self::styles().self::open(__('Upcoming Events','swim-log-and-evaluation'),$a['title']);if(!$events)return$o.'<p>'.esc_html__('No upcoming events are scheduled.','swim-log-and-evaluation').'</p>'.self::close();
		$o.='<div class="swimlog-table-wrap"><table><thead><tr><th scope="col">'.esc_html__('Date','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Event','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Distance','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Stroke','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Current PB','swim-log-and-evaluation').'</th></tr></thead><tbody>';
	...
	public static function log($atts){
		...
		if($annual){
			...
			$o.=self::open(sprintf(__('Swim Log — %d','swim-log-and-evaluation'),$year),$a['title']);
			if(!$rows)return$o.'<p>'.esc_html__('No swim workouts are available for this year.','swim-log-and-evaluation').'</p>'.self::close();
			$o.='<div class="swimlog-table-wrap"><table><thead><tr><th scope="col">'.esc_html__('Month','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Workouts','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Total Distance','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Total Elapsed Time','swim-log-and-evaluation').'</th></tr></thead><tbody>';$wc=0;$tm=0;$ty=0;$te=0;
			...
			$o.='<tr><th scope="row">'.esc_html(sprintf(__('%d Total','swim-log-and-evaluation'),$year)).'</th><td>'.esc_html($wc).'</td><td>'.esc_html(self::total_label($tm,$ty)).'</td><td>'.esc_html(Workout::format_duration($te)).'</td></tr></tbody></table></div>';return$o.self::close();
		}
		...
		$o.=self::open(sprintf(__('Swim Log — %s','swim-log-and-evaluation'),$label),$a['title']);
		if(!$rows)return$o.'<p>'.esc_html__('No swim workouts are available for this month.','swim-log-and-evaluation').'</p>'.self::close();
		$o.='<div class="swimlog-table-wrap"><table><thead><tr><th scope="col">'.esc_html__('Date','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Location','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Distance','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Course','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Elapsed Time','swim-log-and-evaluation').'</th></tr></thead><tbody>';$elapsed=0;
		...
		$workout_count=sprintf(_n('%d workout','%d workouts',count($rows),'swim-log-and-evaluation'),count($rows));
		$o.='<tr><th scope="row">'.esc_html__('Month Total','swim-log-and-evaluation').'</th><td>'.esc_html($workout_count).'</td><td>'.esc_html(self::total_label($t['m'],$t['yd'])).'</td><td>—</td><td>'.esc_html(Workout::format_duration($elapsed)).'</td></tr></tbody></table></div>';return$o.self::close();
	}

	public static function workout($atts){
		self::load();$a=self::attrs($atts,array('id'=>''));$id=absint($atts['id']??0);if(!$id)return self::unavailable();global$wpdb;$t=Database::table('workouts');$base=$wpdb->get_row($wpdb->prepare("SELECT id,user_id FROM $t WHERE id=%d",$id));if(!$base)return self::unavailable();$uid=self::owner($a,$base);if(!self::allowed($uid))return self::unavailable();$w=Workout::get_for_user($id,$uid);if(!$w)return self::unavailable();$perfs=Workout::performances($id,$uid);$o=self::styles().self::open(__('Swim Workout','swim-log-and-evaluation'),$a['title']);
		$o.='<dl><dt>'.esc_html__('Date','swim-log-and-evaluation').'</dt><dd>'.esc_html(self::local_date('F j, Y',$w->workout_start)).'</dd>';if($w->location_name)$o.='<dt>'.esc_html__('Location','swim-log-and-evaluation').'</dt><dd>'.esc_html($w->location_name).'</dd>';$o.='<dt>'.esc_html__('Distance','swim-log-and-evaluation').'</dt><dd>'.esc_html(null!==$w->original_distance?$w->original_distance.' '.$w->original_distance_unit:($w->total_distance_m?$w->total_distance_m.' m':'—')).'</dd><dt>'.esc_html__('Pool','swim-log-and-evaluation').'</dt><dd>'.esc_html(null!==$w->original_pool_length?$w->original_pool_length.' '.$w->pool_length_unit:'—').'</dd><dt>'.esc_html__('Elapsed','swim-log-and-evaluation').'</dt><dd>'.esc_html(Workout::format_duration($w->elapsed_time_ms)).'</dd></dl>';
		if($perfs){$o.='<h3>'.esc_html__('Workout Bests','swim-log-and-evaluation').'</h3><div class="swimlog-table-wrap"><table><thead><tr><th scope="col">'.esc_html__('Distance','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Stroke','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Time','swim-log-and-evaluation').'</th><th scope="col">'.esc_html__('Current PB','swim-log-and-evaluation').'</th></tr></thead><tbody>';foreach($perfs as$p){if($p->stroke==='UNKNOWN')continue;$o.='<tr><td>'.esc_html(self::distance_label((int)$p->distance_value,$p->course_unit)).'</td><td>'.esc_html(self::stroke_name($p->stroke)).'</td><td>'.esc_html(Workout::format_duration($p->duration_ms)).'</td><td>'.($p->is_personal_best?esc_html__('Yes','swim-log-and-evaluation'):'&mdash;').'</td></tr>';}$o.='</tbody></table></div>';}return$o.self::close();
	}
}
