...
final class Event {
	...
	public static function save( $user_id, $data, $id = 0 ) {
		...
		if ( '' === $name ) return new \WP_Error( 'swimlog_event_name', __( 'Event name is required.', 'swim-log-and-evaluation' ) );
		$d = \DateTimeImmutable::createFromFormat( '!Y-m-d', $date );
		if ( ! $d || $d->format( 'Y-m-d' ) !== $date ) return new \WP_Error( 'swimlog_event_date', __( 'Enter a valid event date.', 'swim-log-and-evaluation' ) );
		if ( '' !== $time && ! preg_match( '/^(?:[01]\d|2[0-3]):[0-5]\d$/', $time ) ) return new \WP_Error( 'swimlog_event_time', __( 'Enter a valid event time.', 'swim-log-and-evaluation' ) );
		if ( ! in_array( $distance, self::DISTANCES, true ) ) return new \WP_Error( 'swimlog_event_distance', __( 'Select a supported event distance.', 'swim-log-and-evaluation' ) );
		if ( ! in_array( $course, array( 'm', 'yd' ), true ) ) return new \WP_Error( 'swimlog_event_course', __( 'Select meters or yards.', 'swim-log-and-evaluation' ) );
		if ( ! in_array( $stroke, self::STROKES, true ) ) return new \WP_Error( 'swimlog_event_stroke', __( 'Select a supported stroke.', 'swim-log-and-evaluation' ) );
		if ( $location_id ) {
			require_once SWIMLOG_EVALUATION_DIR . 'includes/class-location.php';
			if ( ! Location::get_for_user( $location_id, $user_id ) ) return new \WP_Error( 'swimlog_event_location', __( 'Select one of your own locations.', 'swim-log-and-evaluation' ) );
		}
		...
		if ( $id ) {
			if ( ! self::get_for_user( $id, $user_id ) ) return new \WP_Error( 'swimlog_event_not_found', __( 'Event not found.', 'swim-log-and-evaluation' ) );
			$result=$wpdb->update($table,$values,array('id'=>(int)$id,'user_id'=>(int)$user_id));
			return false===$result ? new \WP_Error('swimlog_event_save',__( 'The event could not be saved.', 'swim-log-and-evaluation' )) : (int)$id;
		}
		$values['created_at']=$now;
		$result=$wpdb->insert($table,$values);
		return false===$result ? new \WP_Error('swimlog_event_save',__( 'The event could not be saved.', 'swim-log-and-evaluation' )) : (int)$wpdb->insert_id;
	}
